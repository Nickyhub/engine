...
{
    // Search forward from string beginning
    ...
    if (*str)
    {
        ...
        // Search backwards ignore spaces
...
b8 string_to_vec4(char *str, vec4 *out_vector)
{
    ...
    ezero_out(out_vector, sizeof(vec4));
    i32 result = sscanf(
        str,
        "%f %f %f %f",
        &out_vector->x,
...
b8 string_to_vec3(char *str, vec3 *out_vector)
{
    ...
    ezero_out(out_vector, sizeof(vec3));
    i32 result = sscanf(
        str,
        "%f %f %f",
        &out_vector->x,
...
b8 string_to_vec2(char *str, vec2 *out_vector)
{
    ...
    ezero_out(out_vector, sizeof(vec2));
    i32 result = sscanf(
        str,
        "%f %f",
        &out_vector->x,
