...
b8 geometry_system_initialize(u64* memory_requirement, void* state, geometry_system_config config) {
    if (config.max_geometry_count == 0) {
        EN_FATAL("geometry_system_initialize - config.max_geometry_count must be > 0.");
        return false;
    }
    ...
    *memory_requirement = struct_requirement + array_requirement;
    ...
    if (!create_default_geometry(state_ptr)) {
        EN_FATAL("Failed to create default geometry. Application cannot continue.");
        return false;
    }
...
geometry* geometry_system_acquire_by_id(u32 id) {
    ...
    EN_ERROR("geometry_system_acquire_by_id cannot load invalid geometry id. Returning nullptr.");
    return 0;
}
...
geometry* geometry_system_acquire_from_config(geometry_config config, b8 auto_release) {
    ...
    if (!g) {
        EN_ERROR("Unable to obtain free slot for geometry. Adjust configuration to allow more space. Returning nullptr.");
        return 0;
    }

    if (!create_geometry(state_ptr, config, g)) {
        EN_ERROR("Failed to create geometry. Returning nullptr.");
        return 0;
    }
...
void geometry_system_release(geometry* geometry) {
    if (geometry && geometry->id != INVALID_ID) {
        ...
        if (ref->geometry.id == geometry->id) {
            ...
            EN_FATAL("Geometry id mismatch. Check registration logic, as this should never occur.");
        }
        return;
    }

    EN_WARN("geometry_system_acquire_by_id cannot release invalid geometry id. Nothing was done.");
}
...
geometry* geometry_system_get_default() {
    ...
    EN_FATAL("geometry_system_get_default called before system was initialized. Returning nullptr.");
    return 0;
}

b8 create_geometry(geometry_system_state* state, geometry_config config, geometry* g) {
    // Send the geometry off to the renderer to be uploaded to the GPU.
    if (!renderer_frontend_create_geometry(g, config.vertex_count, config.vertices, config.index_count, config.indices)) {
...
void destroy_geometry(geometry_system_state* state, geometry* g) {
    renderer_frontend_destroy_geometry(g);
    g->internal_id = INVALID_ID;
    g->generation = INVALID_ID;
    g->id = INVALID_ID;
...
b8 create_default_geometry(geometry_system_state* state) {
    vertex_3d verts[4];
    ezero_out(verts, sizeof(vertex_3d) * 4);
    ...
    // Send the geometry off to the renderer to be uploaded to the GPU.
    if (!renderer_frontend_create_geometry(&state->default_geometry, 4, verts, 6, indices)) {
        EN_FATAL("Failed to create default geometry. Application cannot continue.");
        return false;
    }
...
geometry_config geometry_system_generate_plane_config(f32 width, f32 height, u32 x_segment_count, u32 y_segment_count, f32 tile_x, f32 tile_y, const char* name, const char* material_name) {
    if (width == 0) {
        EN_WARN("Width must be nonzero. Defaulting to one.");
        width = 1.0f;
    }
    if (height == 0) {
        EN_WARN("Height must be nonzero. Defaulting to one.");
        height = 1.0f;
    }
    if (x_segment_count < 1) {
        EN_WARN("x_segment_count must be a positive number. Defaulting to one.");
        x_segment_count = 1;
    }
    if (y_segment_count < 1) {
        EN_WARN("y_segment_count must be a positive number. Defaulting to one.");
        y_segment_count = 1;
    }

    if (tile_x == 0) {
        EN_WARN("tile_x must be nonzero. Defaulting to one.");
        tile_x = 1.0f;
    }
    if (tile_y == 0) {
        EN_WARN("tile_y must be nonzero. Defaulting to one.");
        tile_y = 1.0f;
    }

    geometry_config config;
    config.vertex_count = x_segment_count * y_segment_count * 4;  // 4 verts per segment
    config.vertices = eallocate(sizeof(vertex_3d) * config.vertex_count, MEMORY_TYPE_DARRAY);
    config.index_count = x_segment_count * y_segment_count * 6;  // 6 indices per segment
    config.indices = eallocate(sizeof(u32) * config.index_count, MEMORY_TYPE_DARRAY);
