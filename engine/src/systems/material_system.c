...
b8 material_system_initialize(u64 *memory_requirement, void *state, material_system_config config)
{
    ...
    {
        EN_FATAL("material_system_initialize - config.max_material_count must be > 0.");
        return false;
    }
    ...
    *memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
    ...
    // Hastable block is after array.
    void *hashtable_block = array_block + array_requirement;

    // Create a hastable for material lookups.
    ...
    // Fill the hashtable with invalid referenes to use as a default.
    ...
    {
        EN_FATAL("Failed to create default material. Application cannot continue.");
        return false;
    }
...
void material_system_shutdown(void *state)
{
    ...
    if (s)
    {
        // Invalidate all materialsin the array
        u32 count = state_ptr->config.max_material_count;
        for (u32 i = 0; i < count; i++)
        {
...
material *material_system_acquire(const char *name)
{
    ...
    string_format(full_file_path, format_str, name, "emt");
    if (!load_configuration_file(full_file_path, &config))
    {
        EN_ERROR("Failed to load material file: %s. Null pointer will be returned.", full_file_path);
        return 0;
    }
...
material *material_system_acquire_from_config(material_config config)
{
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_material_table, config.name, &ref))
    {
        ...
        if (ref.handle == INVALID_ID)
        {
            ...
            {
                EN_FATAL("material_system_acquire - Material system cannot hold anymore materials. Adjust configuration to allow more.");
                return 0;
            }
            ...
            {
                EN_ERROR("Failed to load material '%s'.", config.name);
                return 0;
            }
            ...
            m->id = ref.handle;
            EN_TRACE("Material '%s' does not yet exist. Created, and ref_count is now %i.", config.name, ref.reference_count);
        }
        else
        {
            EN_TRACE("Material '%s' already exists, ref_count increased to %i.", config.name, ref.reference_count);
        }
    ...
    EN_ERROR("material_system_acquire_from_config failed to acquire material '%s'. Null pointer will be returned.", config.name);
    return 0;
}
...
void material_system_release(const char *name)
{
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_material_table, name, &ref))
    {
        if (ref.reference_count == 0)
        {
            EN_WARN("Tried to release non-existent material: '%s'", name);
            return;
        }
        ref.reference_count--;
        if (ref.reference_count == 0 && ref.auto_release)
        {
            ...
            EN_TRACE("Released material '%s'., Material unloaded because reference count=0 and auto_release=true.", name);
        }
        else
        {
            EN_TRACE("Released material '%s', now has a reference count of '%i' (auto_release=%s).", name, ref.reference_count, ref.auto_release ? "true" : "false");
        }
    ...
    {
        EN_ERROR("material_system_release failed to release material '%s'.", name);
    }
}
...
b8 load_material(material_config config, material *m)
{
    ...
    {
        m->diffuse_map.use = TEXTURE_USE_DIFFUSE;
        m->diffuse_map.texture = texture_system_acquire(config.diffuse_map_name, true);
        if (!m->diffuse_map.texture)
        {
            EN_WARN("Unable to load texture '%s' for material '%s', using default.", config.diffuse_map_name, m->name);
    ...
    {
        EN_ERROR("Failed to acquire renderer resources for material '%s'.", m->name);
        return false;
    }
...
void destroy_material(material *m)
{
    EN_TRACE("Destroying material '%s'...", m->name);
...
b8 create_default_material(material_system_state *state)
{
    ezero_out(&state->default_material, sizeof(material));
    ...
    state->default_material.diffuse_map.use = TEXTURE_USE_DIFFUSE;
    ...
    {
        EN_FATAL("Failed to acquire renderer resources for default texture. Application cannot continue.");
        return false;
    }
...
{
    efile f;
    if (!file_open(&f, path, FILE_MODE_READ, false))
    {
        EN_ERROR("load_configuration_file - unable to open material file for reading: '%s'.", path);
        return false;
    }
    ...
    u64 line_length = 0;
    u32 line_number = 1;
    while (file_read_line(&f, 511, &p, &line_length))
    {
        // Trim the string.
        ...
        if (equal_index == -1)
        {
            EN_WARN("Potential formatting issue found in file '%s': '=' token not found. Skipping line %ui.", path, line_number);
            line_number++;
            continue;
        }
        ...
        ezero_out(raw_var_name, sizeof(char) * 64);
        ...
        char raw_value[446];
        ezero_out(raw_value, sizeof(char) * 446);
        ...
        else if (strings_equali(trimmed_var_name, "diffuse_colour"))
        {
            ...
            {
                EN_WARN("Error parsing diffuse_colour in file '%s'. Using default of white instead.", path);
        ...
        ezero_out(line_buf, sizeof(char) * 512);
        line_number++;
    }
